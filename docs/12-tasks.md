...
| `memmap` | in the `check` group — [`tools/memmap.py`](../tools/memmap.py) is the one machine-readable memory map, the same arrangement `opcodes.py` has for the encoding. It verifies itself against the equates every `sw/*.asm` actually declares, and refuses two names on one byte of page 0. **Import from it rather than writing an address down twice**: `sim/test_interp.py` used to carry its own `VARS = 0x0040`, `sim/build_basic.py` its own `ORG`/`TOP` — and `sim/tb/cool8_boot_tb.v` its own `$8104` for the boot banner, which went stale at [D70]'s map move (2026-08-13) and stayed stale through [D82]: the `boot` job failed for four days and nobody saw, because map moves are software work and the `rtl` group was not in the loop. A testbench cannot import, so the rule there takes its other form, the one `+stopat` already used: the address goes in as a plusarg (`+banner=`, `+moncy=`), read out of the assembled `boot.asm` by `sim/test_boot.py`, and a missing plusarg is a hard failure rather than a default |
...
| `action` | the CoolAction! compiler of [15-action.md](15-action.md): one program per construct read back by symbol, the Byte sieve's clocks beside compiled BASIC's, `demos/primes.act` at the UART, what the compiler refuses -- and its assembler against `tools/cool8asm.py`: every program byte for byte, and all 491 encodings through both. **The library on the hardware**: every routine of `sw/libaction.act` called and the machine read back -- VRAM, palette, sprites, sound, the registers -- and `Line` against `LINE` over `test_run.py`'s fan, the whole mode 4 frame compared. **The nine ports against their originals**: each `demos/name.act` runs beside its `name.bas` to the same frame wait, `RND` call or key wait, and what the machine holds -- VRAM, palette, text map, voices, registers -- must be identical; the clocks are printed beside each other where the CPU is what is measured. **`KeyPoll`/`KeyHeld`** from a fed FIFO, on the bare machine and `SYS`'d from a booted BASIC with its interrupt handler running (D101), **KEYTEST** from the flash-booted disc, **the loader** (D102) putting RAINBOW.PRG at $1400 from the disc's catalogue and resetting the machine after it, and **Ms. Cool-Man** played by an autopilot -- and started from BASIC with a real space bar -- -- the title frame, dots, the pill, a ghost eaten and home, a death, two levels cleared into the blue maze, the fruit, the sprite engine's overrun flag -- which skips, loudly, without its art part. `--profile` prints the sieve's clocks by loop, `--profile line` the library's `Line` over the fan |
...
`mkactdata --check`, `cool8kbd --check` and the emulator's self-test —

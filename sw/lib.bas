...
' decodes them. Twenty CONSTs used to be written out here ([D67]).
INCLUDE "io.bas"

' ...and the same names at the assembly level, for the inline ASM blocks
' below, which store to these registers directly. The include is
' idempotent and emits no bytes -- it is equates only -- so it costs
' nothing wherever it lands in the code stream.
ASM
        .include "io.asm"
END ASM

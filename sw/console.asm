...
;
; The cell map at $A000 is the truth -- 80x32 cells of char+attribute,
; stride 160 -- and it is what the editor reads a line back out of. In
...
; << 8)` compiled to a sixteen-bit shift loop. The stride is 160 now
; ([D70]) and the map is page-aligned, so the row is still one byte:
; `r * 160 = (r * 5) << 5`, and `r*5 < 160` always fits in eight bits,
; so the high byte is `(r*5) >> 3` and the low byte `((r*5) AND 7) << 5`
; -- no sixteen-bit arithmetic at any point. `con_row` is cheaper still
; while nothing needs the general form; see the comment on it.
;
...
; version of it has, because the map began at $8000. During [D70] this
; briefly sat at $A41B, the low byte $1B was silently dropped, and every
; cell landed 27 bytes low on top of the stack: a corrupted return
; address and a machine that runs off into nothing. The hardware no
; longer constrains the base at all ([D69]), so this constraint is
; con_row's alone, and one add is worth honouring it for.
; SCREEN and CSTRIDE come from sw/sysbot.asm, generated by
; tools/memmap.py -- the map is packed under system storage rather than
; sitting in the middle of RAM, and 32 rows at 160 is 5120 bytes against
; the 8192 a power-of-two stride needed ([D70]). Written down here once
; and the boot ROM had its own copy; that is how the monitor came to
...
;
; The whole of screen addressing, and it is short **without** a MUL even
; though the stride is 160, because 160 is 5 * 32: the product is
; `q = 5r`, then `q << 5`. For r < 32, `q < 160` fits in a byte, so the
; address's high byte is `q >> 3` and its low byte `(q AND 7) << 5` --
; three adds and six shifts, all eight-bit, and X is never spilled.
;
; [D30] chose a 256-byte stride precisely to make this one add, and the
; map cost 8,192 bytes for it. [D69]'s map-origin register lifted the
; alignment and the power-of-two stride together, so [D70] took the
; 3,072 bytes back and paid for them here: about 20 cycles a row, on a
; routine `con_cell` already caches through `CROWA`. See the note above
; CSCRN for why the base must still be page-aligned.
con_row:
        LD   R1,[CTOP]
        ADD  R0,R1
        AND  R0,#31

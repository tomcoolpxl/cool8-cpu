...
; The registers come from sw/io.asm, generated by tools/ioregs.py.
; Include-once, so it does not matter that boot.asm asks for it too.
        .include "io.asm"

; SCREEN and CSTRIDE, generated by tools/memmap.py. This module had its
; own `SCREEN = $8000` and its own "stride 256" through [D70], so the
; whole monitor drew into user RAM while the display read $A000 -- and
; the gate for this file talks over the serial line, so it passed.
        .include "sysbot.asm"
...
; the window rather than the text -- see `scroll`.
;
; The stride is 160 ([D70]), so the row is not the high byte any more.
; It is still no MUL: 160 is 5*32, `5r` fits in a byte for r < 32, and
; the row offset is `(5r) << 5` -- high `5r >> 3`, low `(5r AND 7) << 5`.
; Same arithmetic as con_row in sw/console.asm, and the same reason.
mrowa:
        LD   R1,[vtop]
        ADD  R0,R1
        AND  R0,#31
        ...
        SHR  R0
        SHR  R0
        ADD  R0,#>SCREEN        ; page-aligned, so this is one add
        MOV  YH,R0
        MOV  YL,R1
        RET

; Y = the cell address of column `cx` on screen row `cy`.
;
; A column cannot reach 128, so `cx * 2` cannot carry out of a byte and
; ADDW finishes it -- but the row offset already uses the low byte, so
; this has to be a 16-bit add rather than a store into YL.
scraddr:
        PUSH R0
        PUSH R1
        LD   R0,[cy]
...
; documentation was missing. The hardware wraps its row pointer against
; an explicit map origin now ([D69]) rather than against `base & ~mask`,
; which is what lifted the alignment and the power-of-two stride -- but
; the origin is latched from the mode preset, so a base walked past the
; end of the map still shows rows that are not there. Keeping `vtop`
; masked to 0..31 is what keeps the base inside the map.
;
...
scroll: PUSHW Y
        ...
        ; The window's top-left cell, which is what VID_BASE points at.
        ; **Both bytes now**: at stride 160 the low byte is only zero on
        ; every eighth row, and writing just the high one moved the
        ; window in 256-byte steps through a 160-byte grid.
        CLR  R0
        CALL mrowa              ; mrowa adds vtop itself, so ask for row 0
        MOV  R0,YL
        ST   [VID_BASE_L],R0
        MOV  R0,YH
        ST   [VID_BASE_H],R0

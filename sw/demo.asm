...
; decodes them. This file used to carry seventeen `.equ`s of its own,
; three of them under private names (`VP_ADDR_L` for `VRAM_ADDR_L`)
; which is how one address came to have three spellings across the
; tree ([D67]).
        .include "io.asm"
...
vp_clear:
        ...
        CLR   R0
.outer: MOV   R2,#0
.inner: ST    [VRAM_DATA],R0              ; the port auto-increments
        SUB   R2,#1
        BNE   .inner
        SUB   R1,#1
        BNE   .outer
...
.cell:  MOV   R0,R1
        ...
        ST    [VRAM_DATA],R0              ;   attr[5:4] would pick patterns
        SUB   R1,#1
        BNE   .cell

#!/usr/bin/env python3
"""Run the M2 gate corpus through the machine.
...
def load(*sources):
    ...
    for src in sources:
        ...
        if a.errors:
            ...
            sys.exit(1)
        base, img = a.image()
        m.bus.mem[base:base + len(img)] = img
        syms.update(a.syms)
    return m, syms


def call(m, addr, max_instr=3_000_000, **state):
    """Run one routine to its return, on the machine RAM carries between
    calls — a fresh register file each time, exactly as before."""
    c = m.cpu
    c.r = [0, 0, 0, 0]
    c.x = c.y = c.f = 0
    c.halted = False
    ...
    for k, v in state.items():
        if k.startswith("r") and len(k) == 2:
            ...
            setattr(c, k, v)
    # push the sentinel return address, as _push16 laid it down
    c.sp = STACK - 2
    m.bus.mem[STACK - 2] = SENTINEL & 0xFF
    m.bus.mem[STACK - 1] = SENTINEL >> 8
    c.pc = addr
    if m.run(until=SENTINEL, budget=max_instr) != "until":
        raise RuntimeError(f"routine at ${addr:04X} never returned")
    return c


def main():
    m, S = load("sw/lib.asm", "sw/gfx.asm", "sw/frames.asm")
    bus = m.bus
    ...
    bus.load(0x2000, src)
    call(m, S["memcpy"], x=0x2000, y=0x2100, r0=64)
    check("memcpy", bytes(bus.mem[0x2100:0x2140]) == src)

    call(m, S["memset"], y=0x2200, r0=0xAB, r1=32)
    check("memset", bytes(bus.mem[0x2200:0x2220]) == b"\xAB" * 32)

    c = call(m, S["memcmp"], x=0x2000, y=0x2100, r0=64)
    ...
    for a, b in ((5, 5), (5, 6), (6, 5), (0x0100, 0x00FF), (0xFFFF, 0)):
        put16(0x2600, a)
        put16(0x2610, b)
        c = call(m, S["cmp16"], x=0x2600, y=0x2610)
    ...
    for _ in range(300):
        ...
        c = call(m, S["mul16"], r0=a & 0xFF, r1=a >> 8,
                 r2=b & 0xFF, r3=b >> 8)
        got = c.r[0] | (c.r[1] << 8)
        if got != (a * b) & 0xFFFF:
            bad += 1
    ...
    for x, y in ((0, 0), (7, 0), (8, 1), (255, 199), (100, 50)):
        c = call(m, S["pixel_addr"], r0=x, r1=y)
    ...
    call(m, S["blit8_mask"], x=0x2900, y=0xB700)
    check("blit8_mask",
    ...
    def cdecl(name, *args):
        ...
        for a in reversed(args):
            ...
            bus.mem[sp] = a & 0xFF
        sp -= 2
        bus.mem[sp] = SENTINEL & 0xFF
        bus.mem[sp + 1] = SENTINEL >> 8
        c.sp = sp
        c.pc = S[name]
        if m.run(until=SENTINEL, budget=500_000) != "until":
            raise RuntimeError(name + " never returned")
        return c.r[0]

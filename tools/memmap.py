...
SYSEND = CSTK - 1
SCREEN = 0x8000             # the text map: 80 cells a row, 32 rows
...
def regions():
    ...
    return [
        ...
        (SCREEN_END + 1, ORG - 1, "HEAP",
         "arrays and strings, down from the image; grows as BASIC shrinks"),
...
def org_asm(write=False):
    ...
    head = [
        ...
        "HEAPTOP = $%04X                 ; the heap's first byte, growing down" % (o - 1),
        "HEAPBOT = $%04X                 ; the top of the text map, which is" % (0x8000 + 160 * 32),
